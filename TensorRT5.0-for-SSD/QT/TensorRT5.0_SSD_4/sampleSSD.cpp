...
static const int kINPUT_H = 224;        // Input image height
static const int kINPUT_W = 224;        // Input image width
static const int kOUTPUT_CLS_SIZE = 4;  // Number of classes
...
struct Param
{
    MODE modelType{MODE::kFP16}; // Default run FP32 precision
} params;
...
static const std::vector<std::string> kDIRECTORIES{"models/", "models/"};                                                                                                                                                                          // Data directory
const std::string gCLASSES[kOUTPUT_CLS_SIZE]{"background", "car", "person", "moto"}; // List of class labels
...
// Visualization
const float kVISUAL_THRESHOLD = 0.99f;
...
int main(int argc, char** argv)
{
    ...
    printf("timeStart:%ld us\n",1000000*timeStart.tv_sec+timeStart.tv_usec);
    ...
    caffeToTRTModel("deploy.prototxt",
                    "Vehicle-mounted_Fc7_iter_17000.caffemodel",
                    std::vector<std::string>{kOUTPUT_BLOB_NAME0, kOUTPUT_BLOB_NAME1},
                    N, params.modelType, &trtModelStream);

    std::vector<std::string> imageList = {"ir3.ppm"}; // Input image list
    ...
    struct timeval StartReadppms;
    gettimeofday(&StartReadppms,NULL);
    printf("StartReadppms:%ld us\n",1000000*StartReadppms.tv_sec+StartReadppms.tv_usec);
...
//    trtModelStream->destroy();
    ...
    struct timeval EndProcessing20ppms;
    gettimeofday(&EndProcessing20ppms,NULL);
    printf("EndProcessing 1 ppms:%ld us\n",1000000*EndProcessing20ppms.tv_sec+EndProcessing20ppms.tv_usec);

    printf("Processing 1 ppms time:%ld us\n",(1000000*EndProcessing20ppms.tv_sec+EndProcessing20ppms.tv_usec) - (1000000*StartReadppms.tv_sec+StartReadppms.tv_usec));
...
//    delete[] keepCount;
    ...
    printf("timeEnd:%ld us\n",1000000*timeEnd.tv_sec+timeEnd.tv_usec);

    printf("program runtime:%ld us\n",(1000000*timeEnd.tv_sec+timeEnd.tv_usec) - (1000000*timeStart.tv_sec+timeStart.tv_usec));
